...
function Products() {
  ...
  const handleSkip = (skipLimit) => {
    // Next button skipLimit =  4
    // Prev button skipLimit = - 4
    setSearchParams((prev) => {
      prev.set("skip", Math.max(skip + skipLimit, 0));
      return prev;
    });
  };
              ...
              <select
                ...
                onChange={(e) => {
                  setSearchParams((prev) => {
                    prev.set("skip", 0);
                    prev.delete("q", 0);
                    prev.set("category", e.target.value.replace(/\s+/g, ""));
                    return prev;
                  });
                }}>
                <option>Select category</option>
                {categories?.map((category) => (
                  <option key={category} value={categories.slug}>
                    {category.name}
                  </option>
                ))}
              </select>
